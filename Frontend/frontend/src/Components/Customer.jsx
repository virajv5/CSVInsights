...
function Customer() {
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      await axios.post('http://127.0.0.1:8000/project/upload-Customer-csv/', formData, {
        headers: {
  ...
  const handleGetAnswer = async (e) => {
    ...
    try {
      const response = await axios.post('http://127.0.0.1:8000/project/generate-customer-query/', { text: question });
